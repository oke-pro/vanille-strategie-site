...
export default function CabinetMessageThreadPage({
  params,
}: {
  ...
  const [clientInfo, setClientInfo] = useState<{ name: string } | null>(null);
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col" style={{ height: "calc(100vh - 10rem)" }}>
      {/* Header */}
      <div className="mb-4">
        <Link href="/cabinet/messages" className="text-sm text-slate-500 hover:text-slate-700">
      ...
      <div className="flex-1 overflow-y-auto bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-3">
        {messages.length === 0 ? (
          <div className="text-center text-slate-500 text-sm py-8">
            Aucun message. Envoyez le premier message a ce client.
          </div>
        ) : (
          messages.map((msg) => {
            const isStaff = msg.sender.role === "staff";
            return (
              <div
                key={msg.id}
                ...
                >
                  ...
                  <p
                    className={`text-[10px] mt-1 ${
                      isStaff ? "text-blue-200" : "text-slate-400"
                    }`}
                  >
